Name = "aletheiaBackgroundSelector"
NamePretty = "Aletheia Background Selector"
Cache = false
...
function GetEntries()
  ...
  local theme_name_file = io.open(home .. "/.config/aletheia/current/theme.name", "r")
  ...
  local dirs = {
    home .. "/.config/aletheia/current/theme/backgrounds",
  }
  if theme_name then
    table.insert(dirs, home .. "/.config/aletheia/backgrounds/" .. theme_name)
  end
      ...
      for background in handle:lines() do
        ...
        if filename and not seen[filename] then
          ...
          table.insert(entries, {
            ...
            Value = background,
            Actions = {
              activate = "aletheia-theme-bg-set " .. ShellEscape(background),
            },
            Preview = background,
            PreviewType = "file",
          })
